...
SINTESI = [
    ...
    "Il modello di giugno (vostro documento del 15/06) aveva tre cose buone che la bozza ha perso — pagamento prima dell'ingresso, verifica di solvibilità, cauzione psicologica — e una che va tenuta fuori: il divieto di minori.",
    ...
    "Valutazione: la bozza vale 5/10 come scheletro (impianto giusto, lessico sbagliato, sei vuoti). Il testo rivisto secondo questa tabella, prima della validazione legale, vale 8/10. Fatto quando: la proprietà ha preso le nove decisioni, il legale ha validato gli otto punti, e il primo contratto è firmato con i quattro allegati.",
]
...
perche=[
 "Il file è ancora nella forma in cui l'assistente AI lo ha prodotto, domanda iniziale compresa. Se arriva così a un ospite, a un ufficio acquisti o all'avvocato di controparte, la credibilità del Veliero cala prima della prima clausola.",
...
perche=[
 "«Ospitalità» è il termine con cui la L.R. 16/2004 e la giurisprudenza qualificano il rapporto in struttura ricettiva, ed è la stessa parola che la proprietà aveva scelto nel modello di giugno («contratto di ospitalità temporanea», vostro documento del 15/06). «Alloggio e prestazione servizi» è corretto ma generico.",
...
perche=[
 "«Servizi alberghieri multimediali» non ha alcun significato tecnico: è un'espressione tipica dei testi generati automaticamente, e un avvocato che la legge dubita di tutto il resto.",
...
perche=[
 ...
 "L'invariabilità per dodici mesi è un argomento di vendita — «nessun conguaglio, nessuna sorpresa» (nostra analisi del 26/08) — e va scritta; l'unica eccezione dichiarata è la franchigia consumi, che così resta trasparente.",
 "Attenzione all'aritmetica dell'IVA: «1.100 € IVA inclusa» vuol dire circa 1.000 € netti per la struttura; se il conto economico era fatto su 1.100 netti, il prezzo al pubblico è 1.210 €. Da chiarire prima di comunicare qualunque cifra.",
...
proposta=[
 ...
 "[PROPRIETÀ] Le frequenze di pulizia e biancheria vanno decise, non lasciate «opzionali». Raccomandazione: pulizia ogni 15 giorni, biancheria mensile, piccola manutenzione inclusa (nostra analisi del 26/08).",
],
perche=[
 ...
 "Wi-Fi: l'upload dentro un appartamento non è mai stato misurato (analisi di scostamento del 26/08). Non si promettono numeri; si promette un tempo di ripristino, che costa poco e per uno smart worker vale molto.",
...
proposta=[
 "§ 4.1 A garanzia delle obbligazioni assunte e dei danni all'unità, agli arredi e alle dotazioni, l'Ospite versa alla firma un deposito di € [1.100 — pari a una mensilità], che non produce interessi.",
...
perche=[
 ...
 "Artt. 1783–1785-quater c.c.: l'albergatore risponde delle cose portate dal cliente in albergo, fino a cento volte il prezzo giornaliero dell'alloggio, e i patti che escludono o limitano questa responsabilità sono nulli. Uno smart worker porta con sé attrezzatura per diverse migliaia di euro, per un anno: la clausola serve a informare, e la polizza RC della struttura va verificata con l'assicuratore per i soggiorni lunghi. La bozza non ne parla. [LEGALE]",
 "Minori: il modello di giugno vietava «di introdurre minori senza autorizzazione» e lo stesso documento riconosceva che escludere le famiglie è illegale (nostra analisi del 26/08). La bozza del 31/08 ha tolto la clausola: va tenuta fuori. Il limite legittimo è la capienza autorizzata.",
]),
...
proposta=[
 ...
 "[PROPRIETÀ] Periodo minimo (3 mesi) e preavviso (60 giorni) da confermare. [LEGALE] 6.2, 6.3 e 6.4 da validare; anche in RTA l'allontanamento di chi rifiuta di uscire non è un atto materiale immediato: la clausola rende rapida e solida l'azione legale, non la sostituisce (nostra analisi del 26/08).",
],
perche=[
 "Due o tre mesi via PEC — che un privato raramente ha — è attrito per lo smart worker e contraddice il messaggio commerciale della stessa proprietà: «si esce con trenta giorni, nessun vincolo di durata» (nostra analisi del 26/08). Sessanta giorni con e-mail confermata è lo standard del corporate housing; una penale certa e limitata (una mensilità) regge in giudizio meglio di un preavviso lungo che un consumatore contesterà come vessatorio (Codice del Consumo, art. 33, c. 2, lett. e-f).",
...
proposta=[
 "§ 8.1 (verso persone fisiche) Per le controversie con un Ospite consumatore è competente il giudice del luogo di residenza o domicilio elettivo dell'Ospite (art. 33, c. 2, lett. u, Codice del Consumo e giurisprudenza costante sul foro del consumatore). Le parti si impegnano a tentare una composizione bonaria prima di adire il giudice.",
...
perche=[
 ...
 "Lo smart worker firma da Milano, non in reception: è un contratto a distanza. Il diritto di ripensamento di 14 giorni non si applica all'alloggio con periodo specifico, ma va scritto: una clausola esplicita evita la contestazione al primo mese. [LEGALE]",
...
B2B_INTRO = ("Il piano del 31/08 dà priorità alla landing Corporate Workforce B2B: aziende e agenzie che alloggiano personale "
             ...
             "proprietà prima di scrivere la landing Corporate: sono il contenuto della pagina (nostra analisi del 26/08).")
...
LEGALE = [
 ...
 "Doppia sottoscrizione (artt. 1341-1342 c.c.) e verifica delle clausole alla luce del Codice del Consumo (artt. 33-36 e 59).",
...
def build_docx(path):
    ...
    for row in ROWS:
        ...
        p = m.paragraphs[0]; p.paragraph_format.keep_with_next = True; r = p.add_run(row["titolo"]); r.bold = True; r.font.size = Pt(11); r.font.color.rgb = NAVY
        for tg in row["tags"]:
